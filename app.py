...
#         search_modular_factories_in_district(district)
...
async def main():
    ...
    # districts_list = ["Almora", "Bageshwar", "Chamoli", "Champawat", "Dehradun", "Haridwar", "Nainital", "Pauri Garhwal", "Pithoragarh", "Rudraprayag", "Tehri Garhwal", "Udham Singh Nagar", "Uttarkashi"]
    districts_input = st.text_input("Enter districts separated by commas like [Almora, Bageshwara, etc]")
    districts_list = [district.strip() for district in districts_input.split(",")]
    State=st.text_input("Enter state name")

    if st.button("Scrape for Modular Factories"):
        for district in districts_list:
            await search_modular_factories_in_district(keyword, district, State)
